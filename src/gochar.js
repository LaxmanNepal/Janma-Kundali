...
const nak=lon=>{const x=norm(lon),i=Math.floor(x/(360/27)),p=Math.floor((x%(360/27))/((360/27)/4))+1;return{name:NAK[i],pada:p}};
function localToUtc(date,time,tz='Asia/Kathmandu'){
 ...
 const parts=new Intl.DateTimeFormat('en-US',{timeZone:tz,timeZoneName:'longOffset',hour:'2-digit'}).formatToParts(naive);
 const z=parts.find(p=>p.type==='timeZoneName')?.value||'GMT+00:00';
 const q=z.match(/GMT([+-])(\d{2})(?::?(\d{2}))?/);const off=q?(q[1]==='-'?-1:1)*(+q[2]+(+q[3]||0)/60):0;
 return new Date(naive.getTime()-off*3600000);
}
export async function calculateGochar({date,time='12:00',timezone='Asia/Kathmandu',natalMoonSign=null,natalLagnaSign=null}){
 ...
 return{date,time,timezone,utc:utc.toISOString(),engine:'Swiss Ephemeris WASM',sidereal:'Lahiri',planets:transit,sadeSati:sade,dhaiya,focus:{saturn:sat,jupiter:jup},natal:{moonSign:moon,lagnaSign:lagna}};
}
export {RASHIS,PLANETS,signOf,norm};
