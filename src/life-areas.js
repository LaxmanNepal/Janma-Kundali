import { detectYogas } from './yoga.js';
...
function natalSignal(data,area){const asc=Number(data?.ascendant?.sign??0),ps=data?.planets||[],cfg=AREAS[area];let score=50;const hits=[];for(const p of ps){const h=houseOf(p.sign,asc);if(cfg.houses.includes(h)){let v=7;if(cfg.planets.includes(p.name))v+=5;if(p.retrograde)v-=3;if(p.name==='राहु'||p.name==='केतु')v-=2;score+=v;hits.push({planet:p.name,house:h,impact:v})}}return{score:clamp(score),hits}}
function dashaSignal(data,area){const periods=data?.dasha?.periods||[],now=Date.now(),d=periods.find(p=>new Date(p.start)<=now&&new Date(p.end)>now);if(!d)return{score:50,lord:null};const cfg=AREAS[area],f=cfg.planets.includes(d.lord);return{score:clamp(50+(f?14:-4)),lord:d.lord}}
function transitSignal(transit,area){const cfg=AREAS[area];let score=50;const hits=[];for(const p of transit?.planets||[]){if(cfg.houses.includes(Number(p.house))){let v=p.retrograde?-5:5;if(cfg.planets.includes(p.name))v+=4;score+=v;hits.push({planet:p.name,house:p.house,impact:v})}}return{score:clamp(score),hits}}
function yogaSignal(data,area){const yogas=detectYogas(data);let score=0;for(const y of yogas){const text=`${y.name||''} ${y.reason||''}`;if((area==='career'&&/कर्म|करियर|प्रतिष्ठा|सूर्य|शनि|मंगल/.test(text))||(area==='finance'&&/धन|गुरु|शुक्र|लाभ/.test(text))||(area==='marriage'&&/विवाह|शुक्र|चन्द्र/.test(text))||(area==='education'&&/बुध|गुरु|शिक्षा/.test(text))||(area==='business'&&/बुध|शुक्र|मंगल|गुरु/.test(text)))score+=y.importance==='high'?8:4}return clamp(score)}
export function analyzeLifeAreas(data,transit=null,strength=null){const out=Object.entries(AREAS).map(([key,cfg])=>{const natal=natalSignal(data,key),dash=dashaSignal(data,key),tr=transitSignal(transit,key),ys=yogaSignal(data,key);const planetScores=(strength?.planets||[]).filter(p=>cfg.planets.includes(p.name)).map(p=>p.score);const ps=planetScores.length?planetScores.reduce((a,b)=>a+b,0)/planetScores.length:50;const score=clamp(natal.score*.28+dash.score*.18+tr.score*.24+ps*.20+(50+ys)*.10);const hits=[...natal.hits,...tr.hits].sort((a,b)=>Math.abs(b.impact)-Math.abs(a.impact));return{key,label:cfg.label,icon:cfg.icon,score,level:level(score),natalScore:natal.score,dashaScore:dash.score,transitScore:tr.score,planetScore:Math.round(ps),yogaScore:ys,dashaLord:dash.lord,influences:hits.slice(0,4),summary:summary(cfg,score,hits,dash.lord)}});return out.sort((a,b)=>b.score-a.score)}
