...
function d9Sign(p){const sign=Number(p?.sign),deg=Number(p?.degree??0);if(!Number.isFinite(sign)||!Number.isFinite(deg))return null;const nav=Math.min(8,Math.floor((deg/30)*9));const movable=[0,3,6,9].includes(sign),fixed=[1,4,7,10].includes(sign);const start=movable?sign:fixed?(sign+8)%12:(sign+4)%12;return (start+nav)%12}
export function d9Dignity(p){const s=d9Sign(p);return s==null?'neutral':dignityForPlanet(p.name,s)}
function vargaScore(p){return DIGNITY_SCORE[d9Dignity(p)]??55}
...
export function analyzePlanetaryStrength(data){const planets=analyzePlanets(data);return planets.map(p=>{const dignityScore=DIGNITY_SCORE[p.dignity]??55;const houseScoreValue=houseScore(p.house);const d9=d9Dignity(p),vargaScoreValue=vargaScore(p);const relation=relationScore(p.name,planets);const av=ashtakavargaHeuristic(p);const combustionPenalty=p.combust?10:0;const score=clamp(dignityScore*.28+houseScoreValue*.20+vargaScoreValue*.18+relation*.12+av*.12+55*.10+Math.min(20,yogaPlanetBonus(p.name,data))-combustionPenalty);return {...p,dignityScore,dignityLabel:DIGNITY_LABELS[p.dignity]||'सामान्य',houseScore:houseScoreValue,d9Dignity:d9,d9DignityLabel:DIGNITY_LABELS[d9]||'सामान्य',vargaScore:vargaScoreValue,relationScore:relation,ashtakavarga:av,score,summary:`${DIGNITY_LABELS[p.dignity]||'सामान्य'} स्थिति, ${p.house}H भाव र D9 को समर्थनको आधारमा ${score}/100 बल।${p.combust?' अस्त/combust भएकाले score मा सावधानी घटक समावेश छ।':''}`}})}
