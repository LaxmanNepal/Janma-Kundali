...
function init(){
  ...
  const sync=()=>{if(window.__janmaTransitEvents)return;const root=document.getElementById('tiPro');if(!root)return;};
  setInterval(()=>{const root=document.getElementById('tiPro');if(!root)return;},2000);
}
