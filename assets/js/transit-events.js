...
const STEP=86400000;
const signDiff=(a,b)=>(a===b?0:((b-a+12)%12));
...
export async function detectTransitEvents(natal,{from=new Date(),days=365}={}){
  ...
  const end=new Date(start.getTime()+days*STEP);
  const events=[];
  let previous=null;
  for(let t=start.getTime();t<=end.getTime();t+=STEP){
    const at=new Date(t);
    const snap=await calculateCurrentTransits(natal,{at});
